...
int ConvertConv2D(Converter* converter, core::Operation* operation) {
  ...
  // Convert to lyf device tensors and operators
  ...
  lyf_device::Conv2DAttr attr;
  attr.pad_type = lyf_device::PadType::AUTO;
  ...
  attr.group = group;
  attr.fuse_type = ConvertFuseCodeToDeviceFuseType(fuse_code);
  converter->AddOperator(lyf_device::OperatorType::LYF_DEVICE_CONV2D,
