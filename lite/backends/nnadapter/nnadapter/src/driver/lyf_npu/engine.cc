...
Context::Context(void* device, const char* properties) : device_(device) {
  // create the raw context from lyf_device
  if (properties == NULL) {
    NNADAPTER_VLOG(3) << "create context from parameters: " << properties
                      << std::endl;
  }
}
...
int Program::BuildFromModel(core::Model* model, core::Cache* cache) {
  ...
  NNADAPTER_LOG(INFO) << "Origin model:" << std::endl << Visualize(model);
  ...
  // Convert a NNAdapter model to a lyf device graph
  graph_ = std::make_shared<lyf_device::Graph>();
  if (!graph_) {
  ...
  std::vector<lyf_device::Tensor*> output_tensors(output_count);
  ...
  execution_ = std::make_shared<lyf_device::Execution>(graph_.get());
  if (cache->token && cache->dir) {
    if (execution_->Build(&cache->buffer) == lyf_device::StatusType::SUCCESS) {
...
int Program::BuildFromCache(core::Cache* cache) {
  ...
  graph_ = std::make_shared<lyf_device::Graph>(cache->buffer);
  if (!graph_) {
    return NNADAPTER_OUT_OF_MEMORY;
  }
  NNADAPTER_VLOG(3) << "Deserialize a graph from a buffer success.";
  std::vector<lyf_device::TensorAttr> input_attrs, output_attrs;
  NNADAPTER_CHECK(graph_->QueryInputsAndOutputs(&input_attrs, &output_attrs) ==
                  lyf_device::StatusType::SUCCESS);
...
int Program::Execute(uint32_t input_count,
  ...
  std::vector<lyf_device::Argument> input_tensors;
  for (uint32_t i = 0; i < input_count; i++) {
    auto& arg = input_arguments[i];
    ...
    lyf_device::Argument tensor;
    tensor.index = arg.index;
    // Update the shape of the input tensor with the new dimensions
    tensor.shape = ConvertToDeviceDimensions(type.dimensions.data,
  ...
  NNADAPTER_CHECK_EQ(execution_->SetInputs(input_tensors),
                     lyf_device::StatusType::SUCCESS);
  NNADAPTER_CHECK_EQ(execution_->Run(), lyf_device::StatusType::SUCCESS);
  std::vector<lyf_device::Argument> output_tensors;
