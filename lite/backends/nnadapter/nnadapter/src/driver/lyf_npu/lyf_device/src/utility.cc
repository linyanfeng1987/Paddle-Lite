...
int64_t get_tensor_precision_bytes(PrecisionType precision) {
  switch (precision) {
    ...
    case PrecisionType::FLOAT64:
      return 8;
    default:
      LYF_DEVICE_LOG(FATAL) << "Unsupported precision type "
...
void* get_tensor_buffer_address(Tensor* tensor) {
  ...
  if (tensor->length < length) {
    LYF_DEVICE_CHECK(tensor->lifetime == LifeTimeType::TEMPORARY_VARIABLE ||
                   tensor->lifetime == LifeTimeType::OUTPUT);
    if (tensor->buffer) {
      free(tensor->buffer);
    }
    tensor->buffer = malloc(length);
    LYF_DEVICE_CHECK(tensor->buffer) << "Failed to allocate " << length
                                   << " bytes, out of memory!";
...
std::vector<Operator*> sort_operators_in_topological_order(Graph* graph) {
  LYF_DEVICE_VLOG(5) << "graph total tensors: " << graph->tensors_.size();
  LYF_DEVICE_VLOG(5) << "graph input tensors: " << graph->input_tensors_.size();
  LYF_DEVICE_VLOG(5) << "graph output tensors: " << graph->output_tensors_.size();
  LYF_DEVICE_VLOG(5) << "graph total operators: " << graph->operators_.size();
...
int serialize_graph_to_buffer(const Graph& graph,
  ...
  for (auto& op : graph.operators_) {
    ...
    for (auto tensor : op.input_tensors) {
      LYF_DEVICE_CHECK(tensor_to_index.count(tensor)) << "Tensor @" << std::hex
                                                    << tensor << " not found!";
  ...
  LYF_DEVICE_CHECK_EQ(offset, length);
  return StatusType::SUCCESS;
}
...
int deserialize_graph_from_buffer(Graph* graph,
  ...
  for (size_t i = 0; i < operator_count; i++) {
    ...
    for (size_t j = 0; j < input_tensor_count; j++) {
      ...
      LYF_DEVICE_CHECK(tensor_index == -1 ||
  ...
  LYF_DEVICE_CHECK_EQ(offset, buffer.size());
  return StatusType::SUCCESS;
}
