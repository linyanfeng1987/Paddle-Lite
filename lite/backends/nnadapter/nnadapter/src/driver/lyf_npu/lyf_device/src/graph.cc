...
Graph::Graph(const std::vector<uint8_t>& buffer) {
  LYF_DEVICE_CHECK(deserialize_graph_from_buffer(this, buffer) ==
                 StatusType::SUCCESS);
}
...
Operator* Graph::AddOperator(OperatorType type,
  ...
  switch (type) {
    case OperatorType::LYF_DEVICE_CONV2D: {
      ...
      LYF_DEVICE_CHECK_EQ(input_count, 3);   // input, filter and bias
      LYF_DEVICE_CHECK_EQ(output_count, 1);  // output
    } break;
    default:
...
int Graph::IdentifyInputsAndOutputs(
  ...
  for (size_t i = 0; i < input_count; i++) {
    ...
    LYF_DEVICE_CHECK(IsValid(tensor)) << "Failed to find input tensor " << i
                                    << " in graph !";
    LYF_DEVICE_CHECK(tensor->lifetime != LifeTimeType::CONSTANT)
        << "Input tensor should not be a constant tensor!";
    tensor->lifetime = LifeTimeType::INPUT;
  }
  for (size_t i = 0; i < output_count; i++) {
    auto tensor = output_tensors[i];
    LYF_DEVICE_CHECK(IsValid(tensor)) << "Failed to find output tensor " << i
                                    << " in graph !";
    LYF_DEVICE_CHECK(tensor->lifetime != LifeTimeType::CONSTANT)
